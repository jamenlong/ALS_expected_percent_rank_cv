...
def to_long(df, by = ["userId"]): # "by" is the column by which you want the final output dataframe to be grouped by
    ...
    kvs = explode(array([struct(lit(c).alias("variable"), col(c).alias("rating")) for c in cols])).alias("kvs") 
    
    long_df = df.select(by + [kvs]).select(by + ["kvs.variable", "kvs.rating"]).filter("rating IS NOT NULL")
